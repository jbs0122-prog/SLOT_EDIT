...
async function callFunction(fnName: string, body: unknown, authHeader: string, timeoutMs = 90000): Promise<any> {
  ...
  try {
    ...
    if (e.name === "AbortError") throw new Error(`${fnName} timed out after ${timeoutMs}ms`);
    throw e;
  } finally {
    clearTimeout(timer);
  }
...
async function loadKeywordScores(db: any, vibeKey: string, seasonLabel: string): Promise<Map<string, number>> {
...
async function loadDnaContext(db: any, gender: string, bodyType: string, vibe: string, season: string, dnaMode: string): Promise<DnaContext> {
...
async function runPipeline(batchId: string, config: {
  ...
  try {
    ...
    await logMsg(batchId, "system", "start", `MCP Pipeline started — ${vibe} / ${season} / ${gender} (DNA: ${dnaMode})`);
    ...
    if (dna.enabled) {
      await logMsg(batchId, "system", "progress", `DNA Lab active: ${dna.cellCount} cells, ${Object.keys(dna.briefsByLook).length} briefs loaded`);
    }

    await logMsg(batchId, "keywords", "start", `Generating keywords via rule engine + learning data${dna.enabled ? " + DNA Lab" : ""}...`);
    ...
    await logMsg(batchId, "keywords", "success", `${totalKw} keywords across ${lookKeys.length} looks (${kwScores.size} learned scores loaded)${dnaTag}`);
    ...
    await logMsg(batchId, "search", "start", `Searching Amazon for ${lookKeys.length} looks...`);
    ...
    for (const lookKey of lookKeys) {
          ...
          for (const kw of kws) {
            ...
            if (globalSearchCache.has(kw)) {
              ...
              try {
                ...
                await logMsg(batchId, "search", "progress", `[${lookKey}/${slot}] "${kw}" → ${results.length} valid${invalid > 0 ? `, ${invalid} filtered` : ""} (${d.total_filtered ?? 0}/${d.total_raw ?? 0})`);
              } catch (e: any) {
                results = [];
                await logMsg(batchId, "search", "progress", `[${lookKey}/${slot}] "${kw}" failed: ${(e.message || "").slice(0, 80)}`);
              }
            }
      ...
      await logMsg(batchId, "search", "progress", `[Look ${lookKey}] ${Object.values(slotCandidates).reduce((a, b) => a + b.length, 0)} candidates found`);
    }
        ...
        if (!slotCandidates[slot]?.length) {
          for (const other of lookKeys) {
            ...
            if (borrowed?.length) {
              ...
              await logMsg(batchId, "search", "progress", `[Look ${lookKey}/${slot}] Borrowed from Look ${other}`);
              break;
            }
          }
        }
    ...
    await logMsg(batchId, "search", "success", `Search complete: ${globalSearchCache.size} unique API calls`);
    ...
    for (const lookKey of lookKeys) {
      ...
      if (!hasCoreSlots) {
        await logMsg(batchId, "register", "error", `[Look ${lookKey}] Missing core slots, skipping`);
        continue;
      }
    ...
    for (const { lookKey, lookBatchId } of lookRegistrationTasks) {
      const slotCandidates = lookSlotCandidatesMap[lookKey];
      await logMsg(batchId, "register", "start", `[Look ${lookKey}] Analyzing & registering products...`);
      let lookReg = 0;
      ...
      for (let i = 0; i < allProducts.length; i += REGISTER_PARALLEL) {
        const chunk = allProducts.slice(i, i + REGISTER_PARALLEL);
        const results = await Promise.allSettled(chunk.map(async ({ slot, product }) => {
          for (let attempt = 1; attempt <= 2; attempt++) {
            try {
              ...
              await logMsg(batchId, "register", "progress", `[${lookKey}/${slot}] Failed: ${reason.slice(0, 120)}`);
              return false;
            } catch (e: any) {
              if (attempt === 2) {
                await logMsg(batchId, "register", "progress", `[${lookKey}/${slot}] Error: ${(e.message || "").slice(0, 120)}`);
              }
            }
          }
          return false;
      ...
      await logMsg(batchId, "register", "success", `[Look ${lookKey}] Registered ${lookReg} products`);
    ...
    await logMsg(batchId, "nobg", "start", `Extracting flatlays for all looks...`);
    ...
    if (allBgProducts.length > 0) {
      ...
      for (let i = 0; i < allBgProducts.length; i += NOBG_PARALLEL) {
        const chunk = allBgProducts.slice(i, i + NOBG_PARALLEL);
      ...
      await logMsg(batchId, "nobg", "success", `${extracted}/${allBgProducts.length} flatlays extracted${failed > 0 ? ` (${failed} timed out)` : ""}`);
    } else {
      await logMsg(batchId, "nobg", "success", `All products already have flatlays`);
    }

    await logMsg(batchId, "outfits", "start", `Generating outfits for ${lookKeys.length} looks...`);
    ...
    for (const c of outfitCandidates) {
      await logMsg(batchId, "outfits", "success", `[Look ${c.lookKey || "?"}] Score: ${c.matchScore ?? "?"}`);
    }

    await logMsg(batchId, "outfits", "progress", `Generating styling insights (template engine${dna.enabled ? " + DNA briefs" : ""})...`);
    ...
    await logMsg(batchId, "system", "success", `Pipeline complete: ${totalRegistered} products, ${outfitIds.length} outfits`);

  } catch (e: any) {
    const msg = e.message || String(e);
    await updateRun(batchId, { status: "failed", error_message: msg });
    await logMsg(batchId, "system", "error", `Pipeline failed: ${msg}`);
  }
}
...
async function handleDnaStatus(url: URL) {
  ...
  const ruleCountByLook: Record<string, number> = {};
  ...
  return ok({ cells: cellResults, recommendation });
}

async function handlePoll(url: URL) {
  ...
  return ok({ run: runResult.data, logs: logsResult.data || [] });
}
...
async function handleSubmitFeedback(body: any) {
  ...
  return ok({ success: true, accepted: acceptedIds.length, rejected: rejectedIds.length });
}

async function handleLearningInsights(body: any) {
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      if (action === "dna-status") return await handleDnaStatus(url);
      return await handlePoll(url);
    }
